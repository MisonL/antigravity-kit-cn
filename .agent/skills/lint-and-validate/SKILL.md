...
description: 自动质量控制、Lint 与静态分析流程。每次代码修改后都应执行，以确保语法正确与项目规范一致。触发关键词：lint、format、check、validate、types、static analysis。
allowed-tools: Read, Glob, Grep, Bash
---

# 代码检查与校验技能 (Lint and Validate Skill)

> **强制要求：** 每次代码变更后都必须运行相应校验工具。在代码无错误前，不得将任务视为完成。

### 按技术栈执行流程 (Procedures by Ecosystem)
...
4. **修复并重复：** 若 “FINAL AUDIT” 仍失败，禁止提交代码。
