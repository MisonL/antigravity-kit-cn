---
name: tdd-workflow
description: 测试驱动开发 (TDD) 工作流原则。红-绿-重构循环。
allowed-tools: Read, Write, Edit, Glob, Grep, Bash
---

# TDD 工作流 (TDD Workflow)

> 先写测试，后写代码。

---

## 1. TDD 核心循环 (The Cycle)

```
🔴 红 (RED) → 编写一个失败的测试
    ↓
🟢 绿 (GREEN) → 编写最少量的代码使测试通过
    ↓
🔵 重构 (REFACTOR) → 优化代码结构与质量
    ↓
   循环往复...
```

---

## 2. TDD 三大法则 (Three Laws)

1. **除非是为了使一个失败的单元测试通过，否则不允许编写任何生产代码。**
2. **只允许编写刚好能够证明失败的测试（编译失败也算失败）。**
3. **只允许编写刚好能够使当前失败测试通过的生产代码。**

---

## 3. “红 (RED)”阶段准则

### 编写重点

| 关注点                | 示例                         |
| --------------------- | ---------------------------- |
| 行为 (Behavior)       | “应当能够相加两个数字”       |
| 边界空间 (Edge cases) | “应当能够处理空输入”         |
| 错误状态              | “遇到无效数据时应当抛出异常” |

### “红”阶段规则

- 测试**必须**先失败。
- 测试名称应清晰描述预期的行为。
- 每个测试（最好）仅包含一个断言 (Assertion)。

---

## 4. “绿 (GREEN)”阶段准则

### 最少代码原则

| 原则         | 含义                                        |
| ------------ | ------------------------------------------- |
| **YAGNI**    | 你以后才不需要它 (You Aren't Gonna Need It) |
| **最简方案** | 编写能让测试通过的最少代码                  |
| **暂不优化** | 此时只求“跑通”，不求“精炼”                  |

### “绿”阶段规则

- 不要编写多余的代码。
- 暂时不要进行性能优化。
- 仅仅是为了通过测试，不多做一点。

---

## 5. “重构 (REFACTOR)”阶段准则

### 优化方向

| 领域     | 动作                |
| -------- | ------------------- |
| 重复逻辑 | 提取公共函数/代码块 |
| 命名规范 | 使代码意图更加清晰  |
| 代码结构 | 改善组织架构        |
| 复杂度   | 简化逻辑判断        |

### 重构规则

- **所有测试必须保持绿色。**
- 采取小步快跑的增量式改动。
- 每次重构之后都进行提交 (Commit)。

---

## 6. AAA 模式 (Arrange-Act-Assert)

每个测试用例均应遵循：

| 步骤               | 目的                 |
| ------------------ | -------------------- |
| **准备 (Arrange)** | 初始化测试数据与环境 |
| **执行 (Act)**     | 执行被测代码逻辑     |
| **断言 (Assert)**  | 验证结果是否符合预期 |

---

## 7. 何时使用 TDD

| 场景               | TDD 价值                |
| ------------------ | ----------------------- |
| 新功能开发         | 极高                    |
| Bug 修复           | 极高 (先写复现测试)     |
| 复杂逻辑实现       | 极高                    |
| 探索性研究 (Spike) | 较低 (研究完后再补 TDD) |
| UI 布局调试        | 较低                    |

---

## 8. 测试优先级

1. **正常路径 (Happy path)**：最基本的功能流程。
2. **错误处理**：预期的错误反馈。
3. **边缘情况 (Edge cases)**：极端的输入条件。
4. **性能评估**：在大负载下的表现。

---

## 9. 应避免的反模式 (Anti-Patterns)

| ❌ 禁止 (Don't)        | ✅ 推荐 (Do)               |
| ---------------------- | -------------------------- |
| 跳过“红”阶段直接写代码 | 亲眼见证测试失败后再写代码 |
| 先写代码后补测试       | 先写测试作为规格说明       |
| 初期就进行过度工程     | 保持简单，仅满足当前测试   |
| 一个测试包含过多断言   | 每个测试仅验证一个行为     |
| 测试内部实现细节       | 测试对外的行为表现         |

---

## 10. AI 增强型 TDD 协作模式

### 多智能体 (Multi-Agent) 编排视角

- **Agent A**：编写失败的测试 (RED)。
- **Agent B**：编写实现代码以通过测试 (GREEN)。
- **Agent C**：对代码进行专家级调优 (REFACTOR)。

---

## Skills 兼容说明 (最小补充)

- **机制基线**：沿用上游 `.agent/skills/tdd-workflow/SKILL.md`。
- **Codex 适配**：由适配层映射到 `.agents/skills/tdd-workflow/SKILL.md`。
- **注意**：文档层不应替代单元测试框架；仅在此定义 TDD 方法论。
