...
> **编排类技能**：提供核心原则，并引导至特定的子技能（Sub-skill）。
...
无论在何种平台上，所有的游戏都遵循游戏循环（Game Loop）：
...
| **观察者/事件（Observer Pattern）** | 跨系统通信           | 生命值变化 → UI 更新                              |
| **ECS（实体组件系统）**     | 数以千计的同类实体   | RTS（即时战略）单位、大规模粒子                   |
| **命令模式（Command Pattern）** | 撤销、重放、联网同步 | 输入记录                                          |
...
性能预算（Performance Budget）：60 FPS = 16.67ms
...
| AI 类型                  | 复杂度 | 使用场景               |
| ------------------------ | ------ | ---------------------- |
| **FSM（有限状态机）**      | 简单   | 3-5 个状态，行为可预测 |
| **行为树（Behavior Tree）** | 中等   | 模块化，对策划人员友好 |
| **GOAP（目标导向行动规划）** | 高     | 基于规划的呈现式行为   |
...
| 禁止                 | 推荐                               |
| -------------------- | ---------------------------------- |
| 每帧更新所有内容     | 使用事件机制、脏标记（Dirty Flag） |
| 在热点循环中创建对象 | 使用对象池                         |
| 完全不使用缓存       | 缓存常用引用                       |
